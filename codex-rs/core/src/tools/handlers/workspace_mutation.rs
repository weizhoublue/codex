...
impl ToolExecutor<ToolInvocation> for WorkspaceMutationHandler {
    ...
    async fn handle(
        ...
        let current = turn.runtime_workspace.snapshot().await;
        let plan = match plan_workspace_mutation(turn.as_ref(), self.mutation, args.path).await {
            Ok(plan) => plan,
            Err(err) => {
                return workspace_error(err.code, err.message, err.cwd, err.workspace_roots);
            }
        };
        if !plan.changed {
            return workspace_success(/*changed*/ false, plan.cwd, plan.workspace_roots);
        }
        let WorkspaceMutationResult {
            changed: _,
            cwd,
            workspace_roots,
            omitted_workspace_roots: _,
        } = plan;
        let canonical = match self.mutation {
            WorkspaceMutation::SetWorkingDirectory => cwd.clone(),
            WorkspaceMutation::AddWorkspaceRoot => workspace_roots
                .last()
                .cloned()
                .unwrap_or_else(|| current.cwd.clone()),
        };
        let preview = session
